...
% dynamics functions, and 
% (2) to concatenate into a single vector the two
% return values of FDfb.  

% Arguments: (1)The first argument is the model data structure;
% (2)the second is the function handle of the dynamics function to be used;
% (3)and the third is the concatenation of the vectors q, qd, tau and
% (4)(optionally) the ground contact force data (f_gc) from which f_ext can
...
% (5) Any other external perturbation
...
if length(f_gc) > 0
  ...
  for i = unique(b)
    if isempty(f_ext{i})
        ...
        f_ext{i} = f_ext{i} + sum( f_gc(:,b==i), 2 );
    end
  end
% elseif nargin ==3
%   f_ext = {};
